...
Visa-only boundary / 仅限签证：visa-free, e-Arrival and a standalone non-visa eTA are negative route findings. Tell the traveller no visa application is needed under the verified conditions; do **not** open an application case or execute the non-visa authorization through this Skill. Historical screening notes remain for auditability.
...
| Kenya / 肯尼亚 | `ETA` tourism for PRC ordinary passports, subject to live eligibility selector | [screening-only historical note](modules/kenya-tourist-eta-china.md), no visa case | [Immigration eTA application guide](https://etakenya.go.ke/how-to-apply) exempts specified **Chinese diplomatic/official/service** passports, not ordinary passports; verify current route in the official selector. |
| Cambodia / 柬埔寨 | temporary `VISA_FREE` if stay ≤14 days and entry 2026-06-15 to 2026-10-15; otherwise assess `EVISA` | [screening-only historical note](modules/cambodia-tourist-china.md) for waived cases | [Official eVisa portal notice](https://www.evisa.gov.kh/) states time-limited Chinese tourist waiver and e-Arrival obligation. Never auto-charge for a waived route. |
